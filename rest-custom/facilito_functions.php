...
/**
 * Detecta si hay un ttoken en el url y lo decodifica 
 * e inicia sesion automaticamente si no esta logueado aun.
 * 
 * */
function wp_getToken() {
    if (!empty($_GET['token']) && !is_user_logged_in()) {
        $token = $_GET['token'];
		$emailID = $_GET['emails'];
 ...
 * */
 ...
 add_action('template_redirect','wp_addProd');
 
function wp_postLogin() {	 
	  
	 $request = $_GET['cart'];
	 wp_redirect('/finalizar-compra/?var=Mobile&carrito='.$request);
...
function wp_addProd() {	 
	 ...
	 $device = $_GET['var'];
	 $request = $_GET['carrito'];
	 
		 	 	 
	 if(is_page('finalizar-compra') && !is_user_logged_in() && $device == 'Mobile')
	 {	
		WC()->cart = new WC_Cart(); 
		...
		$split = array();
		$split = preg_split('/,/i',$request);
		
		foreach($split as $product)
		{			
			 
				 list($titulo, $valor) = explode(":",$product);
				 if($titulo == "id")
				 {
					 $product_id = $valor;
									 }
				 else if($titulo == "quantity")
				 {
					 $p_qty = $valor;
					 WC()->cart->add_to_cart($product_id, $p_qty);
				 }
